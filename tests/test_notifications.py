...
class TestNotification(BaseTest):
    ...
    @allure.sub_suite("Get notifications list")
    @allure.feature("Notifications")
    @allure.story("User can see existed notifications")
    @allure.title("Get notifications list with params ({case.params})")
    @pytest.mark.smoke
    ...
    def test_get_notification_list(self, case):
        ...
        if case.params.is_read is True:
    ...
    @allure.suite("Notifications")
    @allure.sub_suite("Notification creation")
    @allure.feature("Notifications")
    @allure.story("User can get notification about new like to post")
    ...
    def test_get_post_like_notification(self, case_user, support_user, post_cleaner):
        ...
        with allure.step(f"Check a new notification (post like)"):
            ...
            assert new_notification.actor.id == supporter_user_id, f"Error. Target notification was created by unexpected user. ER/AR: {supporter_user_id}/{new_notification.actor.id}" @ allure.feature(
    ...
    @allure.sub_suite("Notification creation")
    @allure.feature("Notifications")
    @allure.story("User can get notification about new comment to post")
    ...
    def test_get_post_comment_notification(self, case_user, support_user, post_cleaner, comment_cleaner):
        ...
        with allure.step(f"Drop comment to post ({created_post.id}) by support user ({support_user})"):
            created_comment = support_user_api.comments_api.create_comment(created_post.id,
                                                                           payload=CreateCommentPayload(
                                                                               content=self.data_helper.generate_text(
                                                                                   10)))
    ...
    @allure.sub_suite("Notification creation")
    @allure.feature("Notifications")
    @allure.story("User can get notification about a new repost of the post")
    ...
    def test_get_repost_post_notification(self, case_user, support_user, post_cleaner):
        ...
        with allure.step(f"Add repost of the post ({created_post.id}) by support user ({support_user})"):
            created_repost = support_user_api.posts_api.repost_post(created_post.id,
                                                                    payload=CreateRepostPayload(repost_type="repost",
                                                                                                content=self.data_helper.generate_text(
                                                                                                    10)))
    ...
    @allure.sub_suite("Notification creation")
    @allure.feature("Notifications")
    @allure.story("User can get notification about a new follower")
    ...
    @allure.feature("Notifications")
    @allure.story("User can read existed notification")
    ...
    def test_read_notification(self, case_user, support_user, follow_request_cleaner):
        ...
        with allure.step(f"Check notification after read"):
            ...
            case_user_api.notifications_api.get_notification_list(params=GetNotificationsParams(is_read=True)).items[0]
    ...
    def test_read_notification_not_existed(self, case_user):
        ...
        case_user_api.notifications_api.mark_read_notification(prepared_notification_id, expected_success=False,
                                                               status_code=404)
    ...
    @pytest.mark.negative
    @pytest.mark.parametrize("case_user", [
        pytest.param("user_eve", id="attempt to read notification with invalid uuid"),
    ])
    ...
    def test_read_notification_already_read(self, case_user, support_user, follow_request_cleaner):
        ...
        new_notification = \
        case_user_api.notifications_api.get_notification_list(params=GetNotificationsParams(is_read=False)).items[0].id
    ...
    def test_read_notification_of_another_user(self, case_user, support_user, follow_request_cleaner):
        ...
        support_user_api.notifications_api.mark_read_notification(new_notification, expected_success=False,
                                                                  status_code=403)

    @allure.suite("Notifications")
    @allure.sub_suite("Read all notifications")
    @allure.feature("Notifications")
    @allure.story("User can read all notifications by 1 request")
    ...
    def test_read_all_notification_of_another_user(self, case_user, support_user, follow_request_cleaner):
        ...
        unread_count_before: int = len(
            case_user_api.notifications_api.get_notification_list(params=GetNotificationsParams(is_read=False)).items)

        case_user_api.notifications_api.mark_read_all_notifications()

        unread_count_after: int = len(
            case_user_api.notifications_api.get_notification_list(params=GetNotificationsParams(is_read=False)).items)
